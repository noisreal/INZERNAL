...
void utils::unprotect_process() //i tried returning a bool but its just so t h i c k code with 80 variables needed to clean up its just not worth it currently, if it fails then theres bigger problems at some other place for sure
{
	void* token = NULL;
	...
	if (size) token_info = HeapAlloc(GetProcessHeap(), 0, size);
	...
	if (psid_admins) FreeSid(psid_admins);
	if (psid_system) FreeSid(psid_system);
	if (psid_everyone) FreeSid(psid_everyone);
...
uintptr_t utils::find_func_has(std::vector<const char*> to_be_found)
{
	auto mod = global::gt;
